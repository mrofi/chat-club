...
class Chat implements MessageComponentInterface
{
            ...
            default:
                ...
                foreach($this->clients as $client) {
                    if ($this->isDiferentSocketButSameDepartment($from, $client)) {
                        $this->sendMessage($client, $from, $msg);
                    }
                }
                break;
        }
    ...
    protected function sendMessage($socket, $from, $msg) {
        $data = array(
    ...
    protected function isDifferentDepartment($first, $second) {
        return true; //$first->user->department === $second->user->department;
    }

    protected function isDiferentSocketButSameDepartment($first, $second) {
        return true; //$this->isDifferentSocket($first, $second) && $this->isDifferentDepartment($first, $second);
    }
}
